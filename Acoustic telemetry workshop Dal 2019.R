...
# and all fish info and locations have been altered slightly so the data are all inaccurate.
...
perm_map <- ggmap(FLmap, extent='normal')+
  ...
  geom_point(data=stationFishID, aes(x=lon,y=lat,size=dets,col=FishID))
perm_map
...
library(data.table)
dets4 <- DetFilter(dets3, tf=7200, mindelay=60, mindist=2000, maxspeed=3)
head(dets4)


# your choice on which to remove. I'll remove them all as a conservative case:

detsf <- dets4 %>% filter(Min_tag_delay_receiver_filter==1 & Speed_time_filter==1 & min_lag_filter==1 &
                          Pre_tag_filter==1 & Min_tag_delay_filter==1 & Min_tag_delay_distance_filter==1)
head(detsf)

#check out what you're losing:
Filteredout <- dets4 %>% filter(Min_tag_delay_receiver_filter==0 | Speed_time_filter==0 | min_lag_filter==0 |
...
ggmap(FLmap, extent='normal') + 
  ...
  scale_x_continuous(limits=c(-82.6, -80.5))+
  scale_y_continuous(limits=c(24.2, 25.4))
...
#there's a lot better ways to plot this data. Not going into it here
